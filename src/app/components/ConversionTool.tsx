...
export function ConversionTool({
  ...
  async function handleSubmit(e) {
    ...
    if (!(file || mergingFiles.length > 1) || !selectedTool) return;
    ...
    try {
      ...
      if (mergingFiles.length > 1) {
        mergingFiles.forEach((file) => formdata.append(`mergingFiles`, file));
      }

      if (selectedTool.handler === "lockpdf")
        formdata.append("userPassword", e.target.password.value);
      if (selectedTool.handler === "unlockpdf")
        formdata.append("password", e.target.password.value);
      if (selectedTool.handler === "splitpdf")
        formdata.append(
          "splitRange",
          `${e.target.from.value}-${e.target.to.value}`
        );
  ...
  function downloadConvertedFile() {
    if (convertedFile && selectedTool) {
      ...
      link.href = url;
      link.download = file ? `pdfkit-${file.name.split(".")[0]}` : "pdfkit";
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {(file || mergingFiles.length > 0) && (
        <div className="mt-6 text-center text-black">
          ...
          ) : selectedTool.handler === "unlockpdf" ? (
            <form
              onSubmit={handleSubmit}
              className="flex items-center justify-center gap-4 mt-3"
            >
          ...
          ) : selectedTool.handler === "splitpdf" ? (
            <form
              onSubmit={handleSubmit}
              className="flex items-center justify-center gap-4 mt-3 text-black"
            >
              <label
                htmlFor="from"
